...
## Sensitive Files — Frontmatter Only

For any file containing `sensitive: true` in the frontmatter:
- **Do not read the file body** — use only the filename and frontmatter fields
- Extract the title from the filename (remove `.md`, replace hyphens with spaces)
- Use `due`, `status`, and `tags` from frontmatter for planning

```bash
# Check if a file is sensitive before reading body
grep -l "sensitive: true" tasks/*.md
```

Include sensitive tasks in the daily plan normally (due dates and titles are safe). Just never read past the frontmatter delimiter (`---`).
...
## Active Projects
- Project Name (next: next action from file, or "–" if sensitive)
