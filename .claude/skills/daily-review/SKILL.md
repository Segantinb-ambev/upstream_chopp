...
4. Read only the relevant task files identified above
   - **If file has `sensitive: true`: read frontmatter only** — use filename as title, do not read body
5. Update task statuses (`status: done` for completed items)
   - For sensitive files: update only the `status` field in frontmatter, do not rewrite body
