...
projects/               - Ongoing work with next actions
  <project-slug>/
    index.md            - Project file (type: project)
...
Projects live in subfolders: `projects/my-project/index.md`
...
| Type | Key Fields |
|------|------------|
| Task | `type: task`, `environment: personal\|work`, `due: YYYY-MM-DD`, `status: pending\|in-progress\|complete\|cancelled` |
| Project | `type: project`, `environment: personal\|work`, `status: active\|paused\|complete\|archived`, next action in body |
| Person | `type: person`, `environment: personal\|work`, `last-contact: YYYY-MM-DD`, follow-ups in body |
| Idea | `type: idea`, `environment: personal\|work`, description in body |

## Environments

All files have an `environment` field: `personal` or `work`.

- Always set `environment` when creating or updating files
- When the user asks about "work" tasks/projects, filter by `environment: work`; "personal" by `environment: personal`
- When no environment is specified in a request, infer from context or ask if ambiguous
